...
def _start_logger(logfile = 'log.txt', filemode = 'w', detail = False):
    ...
    if detail:
        timeformat = logging.Formatter("%(asctime)s %(msecs)d - %(levelname)s - %(module)s.%(funcName)s(%(lineno)d) - %(message)s [%(processName)s(%(process)d) %(threadName)s(%(thread)d)]" , datefmt='%Y%m%d %H%M%S')
    else:
...
def convert_common_course_names(course_list): #to do
...
def main():
    ...
    idx_faculty_include = [i for i, item in enumerate(course_catalog) if item[headers.index('BUREAU_ID')] in faculty_include or faculty_include == []]
    idx_program_include = [i for i, item in enumerate(course_catalog) if item[headers.index('EDUCATION_CODE')] in program_include or program_include == []]
    idx_language_include = [i for i, item in enumerate(course_catalog) if item[headers.index('COURSELANGUAGE')] in language_include or language_include ==[]]
    #idx_contact_exclude = [i for i, item in enumerate(course_catalog) if item[headers.index('CONTACTHOURS')].startswith(contact_exclude)]
    #combine all filters using sets, to leave only courses which pass all filters
    idx = list(set(idx_ects_min) & set(idx_faculty_include) & set(idx_program_include) & set(idx_language_include))# & set(idx_contact_exclude))
    ...
    free_text_headers = ['SUMMARY', 'COURSECONTENS', 'COURSECONTENSMORE'
                     ,'STUDYGOALS', 'STUDYGOALSMORE', 'EDUCATIONMETHOD',
                     'LITRATURE', 'PRACTICALGUIDE', 'BOOKS', 'READER',
                     'ASSESMENT', 'SPECIALINFORMATION', 'REMARKS']
    ...
    for i, course in enumerate(courses_to_assess):
        ...
        if free_text == '':
            ...
            course_metadata[course_id] = (course[headers.index('COURSE_CODE')].strip(), course[headers.index('YEAR_LABEL')]
            , course[headers.index('COURSE_TITLE')].strip(), course[headers.index('EDUCATION_CODE')], course[headers.index('BUREAU_ID')])
    ...
    print("Courses with no free text found:", len(courses_no_words), ". Therefore", len(keyword_frequency), "courses for keyword analysis.")
    print("These courses contained", len(all_clean_words), "total words, including", len(unique_word_stems), "unique word stems.")
    ...
    header = "CourseCode\tYear\tCourseTitle\tProgramCode\tFaculty\tUniqueKeywords\tKeywords\tWords\tPercent"#\tKwRank1\tKwRank2\tKwRank3\tKwRank4\tKwRank5"
    fout.write(header)
    
#    print("\nShowing most common", words_to_show, "keyword stems each for courses with over", keyword_percent_threshold, "% keyword prevalence.")
#    print(40 * "--")
#    print(header)  
#    print(40 * "--")
    for course_id, stats in word_metrics:
        ...
        line = course_code +'\t'+ course_year +'\t'+ course_title +'\t'+ program_code +'\t'+ faculty +'\t'+ str(unqiue_keyword_count) +'\t'+ str(keyword_count) +'\t'+ str(word_count) +'\t'+ str(keyword_percent)# + '\t' + str(frequent_keywords) 
        fout.write('\n'+line)
#        if len(histogram) > 1 and keyword_percent > keyword_percent_threshold:
#            print(line)
